...
void TSDU::forderivacija()
{
    ...
    atau=20*sqrt(tauy);
    Mtausum+=vrtau/atau;
    integral+=0.5*(dtetaprev+(*teta)-teta0)*dt;     //формула трапеций
...
void TSDU::solve()
{
    ...
    do {RK4();} while(var1[_y]<y2 && var1[_y]>0);
    if(var1[_y]<=hr)            //nil
    goto L1;
    do {RK4();}while(var1[_y]>=y2);
    dt=dt2;
    do {RK4();}while(var1[_y]>ymin);
    L1:;
    te=var1[_t];
    xe=var1[_x];
    ye=var1[_y];
